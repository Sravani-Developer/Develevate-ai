...
test("demo mode supports the main product flows", async ({ page }) => {
  ...
  await expect(page.getByText(/Demo mode is active\. You can test the app without the backend\./)).toBeVisible();
  ...
  await expect(page.getByText(/sign in to save the generated roadmap|roadmap saved to backend|backend unavailable/i)).toBeVisible();
